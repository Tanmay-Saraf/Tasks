...
const CreateBoard = ({ onChange ,fetchBoards}) => {
  ...
  return (
    <div className='fixed inset-0 backdrop-blur-xs flex items-center justify-center bg-black/40 z-50'>
      <form onSubmit={(e) => {
        ...
        <input disabled={loading} autoFocus value={title} onChange={(e) => setTitle(e.target.value)} type="text" placeholder='Board Title' className='w-full shadow-md placeholder:text-neutral-400 text-neutral-600 border border-neutral-200  px-4 py-2 outline-none focus:ring focus:ring-neutral-300 bg-white rounded-xl' />
        <div className='flex justify-center items-center gap-2'>
          ...
          <button type='submit' disabled={loading}  className={`shadow-lg ${loading?'bg-neutral-700':'bg-neutral-800'} text-white px-4 py-2 rounded-xl text-md font-medium cursor-pointer hover:-translate-y-0.5 transition-all duration-300 `}>{loading?'Creating':'Create'}</button>
        </div>
      </form>
    </div>
  )
